...
async function getTopRatedShows(startDate, endDate) {
  try {
    const response = await axios.get(
      `${process.env.TMDB_BASE_URL}/discover/tv?api_key=${process.env.TMDB_API_KEY}&sort_by=vote_average.desc&vote_count.gte=600&first_air_date_year.gte=${startDate}&first_air_date_year.lte=${endDate}&region=US`
    );
    const data = response.data.results.map((result) => {
