...
async function getTopRatedShows(startDate, endDate) {
  try {
    const response = await axios.get(
      `${process.env.TMDB_BASE_URL}/discover/tv?api_key=${process.env.TMDB_API_KEY}&sort_by=vote_average.desc&vote_count.gte=600&primary_release_date.gte=${startDate}-01-01&primary_release_date.lte=${endDate}-12-31&region=US`
    );
    const data = response.data.results.map((result) => {
